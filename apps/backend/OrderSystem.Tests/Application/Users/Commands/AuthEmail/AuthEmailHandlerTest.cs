...
namespace OrderSystem.Tests.Application.Users.Commands.AuthEmail;

public class AuthEmailHandlerTest
{
    private readonly AuthEmailHandler _authEmailHandler;
    ...
    public AuthEmailHandlerTest()
    {
        ...
        // Setup do repositório - GetByEmailAsync
        _mockUserRepository.Setup(u => u.GetByEmailAsync("test@email.com"))
            .ReturnsAsync(_testUser);
        _mockUserRepository.Setup(u => u.GetByEmailAsync("unknown@email.com"))
        ...
        // Instanciar handler
        _authEmailHandler = new AuthEmailHandler(
    ...
    {
        // Arrange
        var command = new AuthEmailCommand("test@email.com", "correctpass");

        // Act
        var result = await _authEmailHandler.Handle(command, CancellationToken.None);
    ...
    {
        // Arrange
        var command = new AuthEmailCommand("unknown@email.com", "anypass");

        // Act & Assert
        await Assert.ThrowsAsync<UserNotFoundException>(
            () => _authEmailHandler.Handle(command, CancellationToken.None)
        );
    }
    ...
    {
        // Arrange
        var command = new AuthEmailCommand("test@email.com", "wrongpass");
    ...
    public async Task Handle_PasswordServiceThrowsException_Propagates()
    {
        ...
        var command = new AuthEmailCommand("test@email.com", "anypass");
